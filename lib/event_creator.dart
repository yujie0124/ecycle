...
class EventCreatorState extends State<EventCreator> {
  ...
  Future _saveNewEvent(BuildContext context) async {
    ...
    if (currentUser != null && this._formKey.currentState.validate()) {
      ...
      Firestore.instance.collection("calendar_events").add({'name': _eventData.title, 'summary': _eventData.summary,
        'time': _eventData.time, 'email': currentUser.email});

      showDialog(
          context: context,
          builder: (_) => new AlertDialog(
            title: new Text("Creating New Event"),
            content: new Text("You have successfully created a new event!"),
            actions: <Widget>[
              FlatButton(
                child: Text('OK'),
                onPressed: () {
